...
export function newtonRefine(poly: Polynomial, deriv: Polynomial, x0: number, iterations = 4): number {
    let x = x0;
    for (let i = 0; i < iterations; i++) {
        const f = evaluatePolynomial(poly, x);
        const fp = evaluatePolynomial(deriv, x);
        if (fp === 0) break;
        const next = x - f / fp;
        if (next <= 0 || Math.abs(next - x) < 1e-12) break;
        x = next;
    }
    return x;
}
