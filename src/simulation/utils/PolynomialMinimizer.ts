...
export function positiveGlobalMinimizer(polynomial: Polynomial, precision = 1e-7): number | null {
    ...
    const secondDerivative = polynomialDerivative(derivative);
    const candidateRoots = findPositiveRoots(derivative, precision).map(r => newtonRefine(derivative, secondDerivative, r));
